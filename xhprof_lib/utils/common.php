...
function displayRuns($resultSet, $title = "")
{
    ...
    while ($row = XHProfRuns_Default::getNextAssoc($resultSet))
    {
        ...
        echo "\t\t<td>{$row['cpu']}</td>";
        echo "\t\t<td raw=\"".$row['wt']."\">".usecToSize($row['wt'])."</td>";
        echo "\t\t<td raw=\"".$row['pmu']."\">".bytesToSize($row['pmu'])."</td>";
        echo "\t\t<td><a href=\"?geturl={$url}\">{$html['url']}</a></td>";
        echo "\t\t<td><a href=\"?getcurl={$c_url}\">{$html['c_url']}</a></td>";
        echo "\t</tr>\n";
    }
    echo "</tbody>\n";
    echo "</table>\n";
...
$(document).ready(function() 
    { 
        $("#box-table-a").tablesorter( {sortList: [], textExtraction: rawextractor} );
    } 
);
</script>
SORTTABLE;
