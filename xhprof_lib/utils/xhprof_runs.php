...
*/
  ...
  */
    public function save_run($xhprof_data, $type, $run_id = null, $xhprof_details = null)
    {
      ...
      $body   = ( isset( $_POST ) ) ? $_POST : [];
      $cookie = ( isset( $_COOKIE ) ) ? $_COOKIE : [];

      $sql['get'] = $this->db->escape(serialize($query));
      $sql['cookie'] = $this->db->escape(serialize($cookie));

          //This code has not been tested
        if (isset($_xhprof['savepost']) && $_xhprof['savepost'])
      {

				$sql['post'] = $this->db->escape(serialize($body));
			} else {
				$sql['post'] = $this->db->escape(serialize(array("Skipped" => "Post data omitted by rule")));
			}
		} else {
			$sql['get'] = $this->db->escape(json_encode($query));
			$sql['cookie'] = $this->db->escape(json_encode($cookie));

	        //This code has not been tested
		    if (isset($_xhprof['savepost']) && $_xhprof['savepost'])
			{
				$sql['post'] = $this->db->escape(json_encode($body));
			} else {
